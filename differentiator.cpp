...
int main() {
    int pos = 0;
    char *buf = "(* (sin x) (sin y))";
    Expression *node_1 = create_tree(buf, &pos);
...
int compare_str_bin(char *value) {
    ...
    if (!strcmp(value, "+")) return ADD;
    if (!strcmp(value, "-")) return SUB;
    if (!strcmp(value, "*")) return MUL;
    if (!strcmp(value, "/")) return DIV;
    if (!strcmp(value, "*")) return MUL;
    return ERROR;
}
